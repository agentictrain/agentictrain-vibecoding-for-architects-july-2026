...
**Status:** Draft for participant and facilitator review  
**Date:** 2026-07-21  
...
| Keyboard and status-announcement checks | 100% pass | Playwright plus participant keyboard check |
| 320-pixel layout checks | 100% pass with no page-level horizontal overflow | Playwright viewport assertion and screenshot review |
| Scope and safety acceptance | Facilitator accepts | Recorded facilitator review |

The PoC is not accepted while the facilitator-approved advisory rule table is absent. This gate is not a product telemetry metric.
...
- `node:test` may test pure modules; Playwright may be a development-only browser test dependency.
...
**FR-3 Scenario.** The participant explicitly selects `warehouse-planning` or `delivery-planning`; neither is selected on hard reset.

**FR-4 Unit system.** The control permits exactly `metric` or `imperial`; `metric` is selected on hard reset. Pressure remains hPa in both systems.

**FR-5 Fetch weather.** The only weather command is Fetch weather. It is disabled until a selected location and scenario exist and while the active attempt is loading.

### P0: Open-Meteo forecast request

**FR-6 Single request.** One Forecast API request obtains all current, hourly, and daily variables for the selected coordinates with `timezone=auto`, `forecast_days=7`, and explicit selected units.
...
**FR-7 Contract boundary.** A `WeatherSignal` is a plain JavaScript object with exactly `metadata`, `current`, `hourly`, and `daily` at the top level. Advisory text never enters the contract.
...
- `evidenceMode` is `live` or `fictional-fallback`.
- `producedAt` is a client-produced ISO 8601 instant, not a provider observation timestamp.
- `unitSystem` is the selected bounded unit system.
- `location` carries geocoding ID or `null`, name, optional `admin1`, country, and finite in-range coordinates.
- `timezone` carries the Open-Meteo IANA name, integer offset, and nullable substitution reason. Fallback uses the selected geocoding timezone; if unavailable/invalid, it uses `UTC`, offset `0`, and the substitution reason `Geocoding timezone unavailable; UTC used for fictional fallback.`
- `source.kind` is `open-meteo` or `workshop-fixture`.
- Live source has `fixtureId=null` and `failedLiveAttempt=false`.
- Fallback source has `fixtureId=weather-fallback-v1` and `failedLiveAttempt=true`.
...
- Codes use `WMO weather interpretation code`; `isDay` uses `boolean`; local times identify the timezone convention.
- Metric uses °C, mm, km/h, hPa, degrees, and percent. Imperial uses °F, inch, mph, hPa, degrees, and percent.

**FR-8 Atomic validation.** Mapping rejects the whole live response when any required block, field, unit, cardinality, timestamp, or alignment is invalid.
...
**FR-9 Distinct views.** Region 2 provides:
...
**FR-10 Visible states.** Region 2 has mutually exclusive empty, loading, success, fallback, and error-with-retry states.
...
**FR-11 Fallback fixture.** `weather-fallback-v1` has fixed values and sequence. Its timestamps are deterministically anchored to fallback creation in the selected geocoding timezone, or UTC when that timezone is missing/invalid. Given the same anchor, unit system, and fixture version, it emits the same object.
...
- Retry live weather.
- UTC was substituted when geocoding timezone was unavailable.
...
Implementation and final acceptance remain blocked until the facilitator supplies and approves a complete rule table covering thresholds, precedence, exact copy, both unit systems, and fallback evidence. The participant and coding agent must not invent these values. Until then, the advisory area states that facilitator-approved rules are unavailable and emits no weather-derived advice.

### P0: Region 3 placeholder

**FR-13 Reserved region.** Region 3 is visibly labeled **Reserved for a future workshop phase** and has no controls, data flow, advisory, or hidden behavior.

### P0: Accessibility and responsiveness

**FR-14 Accessibility.** All controls and view switchers have accessible names. Search follows the combobox/listbox keyboard pattern. Native elements are preferred. Focus remains visible. A polite live region announces search and weather status; urgent failures use one non-duplicated alert. Loading and evidence mode use text and not color alone. Expandable views expose expanded state.

**FR-15 Responsive layout.** From 320 CSS pixels upward, the page has no page-level horizontal overflow. Long place names and timestamps wrap; JSON may scroll only within its bounded viewer. Motion is nonessential and respects reduced motion.

### P0: Stateless lifecycle

**FR-16 Hard reset.** Reload restores an empty location query and selection, no scenario, metric units, empty evidence, no advisory, no status/retry context, and default collapsed views. The app uses no web storage, cookies, IndexedDB, service worker, URL restoration, or application cache.
...
- Fallback is always a failure state and carries `fictional-fallback` provenance.
...
- Client `producedAt` is not an observation/publication timestamp.
...
| Missing geocoding timezone | Selection allowed; fallback uses/discloses UTC |
...
| No payload exists | Raw view states no payload was received |
| Fixture construction/render fails | Error with Retry; no evidence shown |
| Reload | Hard-reset empty state; metric default only |
| Advisory rules absent | No derived advice; explicit facilitator-rule gate |

## Open questions and dependencies

### Blocking

- **Facilitator:** Supply and approve the versioned advisory rule table. This blocks advisory implementation and final acceptance, but not weather retrieval, mapping, evidence views, fallback, or their tests.
...
There is no production deadline or release. Implement this phase in ordered, independently testable slices: shell and state; geocoding; contract mapping; forecast orchestration; fallback; evidence views; browser accessibility/responsiveness; then advisory only after its external rule-table gate clears.
