...
- Decision: Initialize model settings with `https://ollama.com`, `glm-5.2`, and a blank temporary credential.
  Reason: The human explicitly selected the prefilled endpoint and model option; the credential must always start blank.
  Alternatives considered: Leave the endpoint blank; leave all three fields blank.
  Expected impact: The later model-call phase receives the originally approved settings fields and defaults, while reload always clears the credential. This decision was later superseded by the Groq Free tier migration below.
...
- Decision: Put the ollama request and strict response validator in `app/review.js`, loaded as a classic script before `app.js`.
  Reason: The transport and untrusted-response boundary can be tested without DOM setup while the app remains directly openable from `file://`.
  Alternatives considered: Add the request, prompt, and validation to the existing controller file.
  Expected impact: One additional plain-JavaScript file and a narrow Node test surface; no module, dependency, persistence, or build step.
  Human approval still needed: No — explicitly approved by the human.

- Decision: Invalidate and abort the current review whenever weather, scenario, or model settings change.
  Reason: A generated review is valid only for the exact mapped signal, fictional scenario, and configured model request that produced it.
  Alternatives considered: Keep the old review visible with a stale label; allow concurrent requests and accept the last response to finish.
  Expected impact: Stale model responses cannot overwrite current state, and Region 3 returns to a readiness message when an input changes.
  Human approval still needed: No — included in the approved implementation plan.

- Decision: Do not validate or whitelist the model settings field before a review request.
  Reason: The human clarified that the complete set of valid model values is not known to the client.
  Alternatives considered: Require exactly `glm-5.2`; require any non-empty model string.
  Expected impact: The configured model value is forwarded unchanged and the provider determines whether it is valid. The initial model remains a default only.
  Human approval still needed: No — explicitly directed by the human.

- Decision: Replace Ollama Cloud with Groq Free tier and use `openai/gpt-oss-20b` as a replaceable default model.
  Reason: Credential-free preflights proved that Ollama Cloud rejects direct browser requests with HTTP 405, while Groq accepts `Origin: null`, `POST`, `Authorization`, and `Content-Type`. Groq documents a Free tier, free API keys, and JSON Object Mode; a payment method is required only when upgrading to the paid Developer tier. The human approved any model and then required a provider with a free tier.
  Alternatives considered: OpenRouter free variants, Gemini's OpenAI-compatible endpoint, OpenAI, and local Ollama. OpenRouter's free variants are viable but have lower no-credit daily request limits and variable upstream availability; local Ollama is unavailable to students.
  Expected impact: Students can create a free key and call the hosted endpoint from the static `file://` app without a proxy. Browser-held keys remain workshop-only and must be revoked after use.
  Human approval still needed: No — explicitly directed by the human.