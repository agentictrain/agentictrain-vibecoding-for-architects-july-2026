...
const WeatherReviewApi = window.WeatherReview;

if (!WeatherSignalApi) {
  throw new Error('WeatherSignal API failed to load.');
}
if (!WeatherReviewApi) {
  throw new Error('WeatherReview API failed to load.');
}
...
const elements = {
  ...
  reviewRegion: document.querySelector('#review-region'),
  ...
  clearModelSettings: document.querySelector('#clear-model-settings'),
...
function bindEvents() {
  ...
  elements.generateReview.addEventListener('click', handleGenerateReview);
  ...
  elements.openModelSettings.addEventListener('click', openModelSettingsDialog);
  elements.modelSettingsForm.addEventListener('submit', handleSaveModelSettings);
  elements.clearModelSettings.addEventListener('click', handleClearModelSettings);
  elements.cancelModelSettings.addEventListener('click', closeModelSettingsDialog);
  elements.modelSettingsDialog.addEventListener('click', handleModelSettingsBackdropClick);
  elements.modelSettingsDialog.addEventListener('keydown', handleModelSettingsKeydown);
  elements.modelSettingsDialog.addEventListener('cancel', handleModelSettingsCancel);
  elements.modelSettingsDialog.addEventListener('close', restoreModelSettingsFocus);
}

function openModelSettingsDialog(event) {
  modelSettingsOpener = event.currentTarget;
  populateModelSettingsForm();
  elements.modelSettingsDialog.showModal();
  window.requestAnimationFrame(function focusModelSettings() {
    elements.modelBaseUrl.focus();
  });
}

function populateModelSettingsForm() {
  elements.modelBaseUrl.value = state.modelSettings.baseUrl;
  elements.modelName.value = state.modelSettings.model;
  elements.modelApiKey.value = state.modelSettings.apiKey;
}

function handleSaveModelSettings(event) {
...
function handleClearModelSettings() {
  resetReviewState();
  state.modelSettings = { baseUrl: '', model: '', apiKey: '' };
...
function closeModelSettingsDialog() {
  if (elements.modelSettingsDialog.open) {
    elements.modelSettingsDialog.close();
  }
}

function handleModelSettingsCancel(event) {
  event.preventDefault();
  closeModelSettingsDialog();
}

function handleModelSettingsBackdropClick(event) {
  if (event.target !== elements.modelSettingsDialog) {
    return;
  }
  const bounds = elements.modelSettingsDialog.getBoundingClientRect();
...
function handleModelSettingsKeydown(event) {
  if (event.key === 'Escape') {
    event.preventDefault();
    closeModelSettingsDialog();
    return;
  ...
  const focusableElements = Array.from(
    elements.modelSettingsDialog.querySelectorAll('input, button:not([disabled])')
  );
...
function restoreModelSettingsFocus() {
  if (modelSettingsOpener) {
    modelSettingsOpener.focus();
    modelSettingsOpener = null;
  }
}
...
async function handleGenerateReview() {
  if (!canGenerateReview() || state.review.kind === 'loading') {
    return;
  }
  cancelReviewRequest();
  ...
  state.review = { kind: 'loading', data: null, error: '' };
  announce('Generating weather-grounded review.');
  render();

  try {
    const review = await WeatherReviewApi.requestReview(
      state.weather.signal,
      state.scenario,
      state.modelSettings,
    ...
    state.review = { kind: 'success', data: review, error: '' };
    announce('Weather-grounded review generated.');
    render();
  } catch (error) {
    if (error.name === 'AbortError' || requestId !== state.reviewRequestId) {
      return;
    }
    state.review = {
      kind: 'error',
    ...
    announce('Review generation failed. Weather evidence remains available.');
    render();
  } finally {
    if (requestId === state.reviewRequestId) {
...
function resetReviewState() {
  cancelReviewRequest();
  state.reviewRequestId += 1;
  state.review = { kind: 'empty', data: null, error: '' };
}
...
function renderControls() {
  ...
  elements.generateReview.disabled = !canGenerateReview() || state.review.kind === 'loading';
  elements.generateReview.textContent = state.review.kind === 'success'
    ? 'Generate again'
    : 'Generate review';
}
...
function renderScenarioEcho() {
  ...
  elements.scenarioEcho.textContent =
    state.scenario === 'warehouse-planning'
      ? 'Warehouse planning selected.'
      : 'Delivery planning selected.';
}

function canGenerateReview() {
  const hasWeather = (state.weather.kind === 'success' || state.weather.kind === 'fallback') &&
    Boolean(state.weather.signal);
  return Boolean(
    hasWeather && state.scenario && state.modelSettings.baseUrl.trim() &&
    state.modelSettings.model.trim() && state.modelSettings.apiKey
  );
}

function renderReview() {
  elements.reviewRegion.setAttribute('aria-busy', String(state.review.kind === 'loading'));
  if (state.review.kind === 'loading') {
    elements.reviewRegion.replaceChildren(createReviewMessage(
      'Generating review',
      'Waiting for one complete JSON response from Groq.',
      'review-card review-card--muted'
    ));
    return;
  }
  if (state.review.kind === 'success') {
    elements.reviewRegion.replaceChildren(renderReviewResult(state.review.data));
    return;
  }
  if (state.review.kind === 'error') {
    elements.reviewRegion.replaceChildren(createReviewMessage(
      'Review unavailable',
      `${state.review.error} Weather evidence remains visible and unchanged.`,
  ...
  elements.reviewRegion.replaceChildren(createReviewMessage(
    'Not generated yet',
    getReviewReadinessMessage(),
    'review-card review-card--muted'
  ));
}

function getReviewReadinessMessage() {
  if (!state.weather.signal) {
    return 'Load live or fictional weather evidence first.';
  }
  if (!state.scenario) {
    return 'Choose a fictional operational scenario.';
  }
  if (!state.modelSettings.baseUrl.trim()) {
    return 'Add the Groq endpoint in Model settings.';
  }
  if (!state.modelSettings.model.trim()) {
    return 'Add a model name in Model settings.';
  }
  if (!state.modelSettings.apiKey) {
    return 'Add a temporary demo credential in Model settings.';
  }
  return 'Ready to send the mapped WeatherSignal and selected scenario.';
}

function createReviewMessage(headingText, bodyText, className) {
...
function renderReviewResult(review) {
  ...
  stack.className = 'review-card-stack';
  stack.appendChild(createReviewMessage('Summary', review.summary, 'review-card review-card--summary'));
  ['risks', 'actions', 'questions', 'evidence'].forEach(function appendReviewList(field) {
    const article = document.createElement('article');
    const heading = document.createElement('h3');
    const list = document.createElement('ul');
    article.className = `review-card review-card--${field}`;
    heading.textContent = field.charAt(0).toUpperCase() + field.slice(1);
    review[field].forEach(function appendReviewItem(item) {
