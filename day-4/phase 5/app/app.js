...
function handleClearModelSettings() {
  ...
  elements.modelSettingsForm.reset();
  elements.modelBaseUrl.focus();
  render();
}
...
function canGenerateReview() {
  const hasWeather =
    (state.weather.kind === 'success' || state.weather.kind === 'fallback') &&
    Boolean(state.weather.signal);
  return Boolean(
    hasWeather &&
    state.scenario &&
    state.modelSettings.baseUrl.trim() &&
    state.modelSettings.apiKey
  );
}
...
function renderReviewResult(review) {
  ...
  const summary = createReviewMessage('Summary', review.summary, 'review-card review-card--summary');
