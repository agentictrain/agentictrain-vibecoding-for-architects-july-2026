...
const state = {
  ...
  weather: {
    ...
    rawSummaryLabel: 'Raw Open-Meteo response'
  },
  search: {
    isLoading: false,
    results: [],
...
const ui = {
  ...
  scenarioSelect: document.querySelector('#scenario'),
  unitSystemSelect: document.querySelector('#unit-system'),
  ...
  weatherRawSummary: document.querySelector('#weather-raw-summary'),
  weatherActions: document.querySelector('#weather-actions'),
  weatherRetry: document.querySelector('#weather-retry'),
  fallbackBanner: document.querySelector('#fallback-banner'),
  fallbackBannerMessage: document.querySelector('#fallback-banner-message'),
  fallbackTimezoneNote: document.querySelector('#fallback-timezone-note')
};
...
function getForecastUnitParams(unitSystem) {
...
function buildForecastUrl(location) {
  const unitParams = getForecastUnitParams(state.selectedUnitSystem);
  const params = new URLSearchParams({
    ...
    precipitation_unit: unitParams.precipitation_unit,
...
async function fetchWeatherForSelectedLocation() {
  ...
  try {
    ...
    try {
      const safeFallbackTimezone = isValidTimezone(state.selectedLocation.timezone)
        ? state.selectedLocation.timezone
        : 'UTC';

      const fallbackSignal = window.WeatherSignal.createFallbackSignal({
        ...state.selectedLocation,
        timezone: safeFallbackTimezone
      });

      const timezoneFallbackNote = isValidTimezone(state.selectedLocation.timezone)
        ? ''
        : 'UTC fallback because geocoding timezone was unavailable.';

      setWeatherState({
        kind: 'fallback',
        message: 'Live fetch failed. Showing fictional fallback data.',
        sourceUrl: fallbackSignal.sourceUrl,
        rawPayload,
        mappedSignal: fallbackSignal,
        failureMessage: error.message || 'Weather fetch failed.',
        timezoneFallbackNote,
        rawSummaryLabel: rawPayload ? 'Failed live response' : 'Failed live response (no payload received)'
      });
    } catch (fallbackError) {
      setWeatherState({
        kind: 'error',
        message: `Live fetch failed and fallback could not be built: ${fallbackError.message}`,
        sourceUrl: '',
        rawPayload: null,
        mappedSignal: null,
        failureMessage: error.message || 'Weather fetch failed.',
...
function renderReadableWeather(signal, mode) {
  ...
  modeLine.className = 'field-note';
  modeLine.textContent = mode === 'fallback'
    ? 'Evidence mode: fictional fallback'
    : 'Evidence mode: live';

  ui.weatherReadable.append(grid, sourceLine, modeLine);
}
...
function renderWeatherState() {
  ...
  ui.weatherActions.hidden = true;
  ui.fallbackBanner.hidden = true;
  ui.fallbackBannerMessage.textContent = '';
  ui.fallbackTimezoneNote.textContent = '';
  ui.weatherRawSummary.textContent = rawSummaryLabel;
  ...
  if (kind === 'fallback') {
    ui.weatherStatus.textContent = 'Fictional fallback data is shown because live weather failed.';
    ui.weatherSuccess.hidden = false;
    ui.weatherActions.hidden = false;
    ui.fallbackBanner.hidden = false;
    ui.fallbackBannerMessage.textContent = `The live fetch failed (${failureMessage}). Values below are a workshop example and are not actual weather for the selected place.`;
    ui.fallbackTimezoneNote.textContent = timezoneFallbackNote;

    renderReadableWeather(mappedSignal, 'fallback');
    ui.weatherRawJson.textContent = rawPayload
      ? JSON.stringify(rawPayload, null, 2)
      : 'No response payload was received for the failed live request.';
    ui.weatherMappedJson.textContent = JSON.stringify(mappedSignal, null, 2);
    ui.starterStatus.textContent = 'Live fetch failed. Fictional fallback is visible with retry available.';
    return;
  }

  if (kind === 'error') {
    ui.weatherStatus.textContent = 'Weather request failed and fallback is unavailable. Retry is available.';
    ui.weatherError.hidden = false;
    ui.weatherError.textContent = message;
    ui.weatherEmpty.hidden = false;
    ui.weatherActions.hidden = false;
    ui.starterStatus.textContent = 'Weather fetch and fallback failed. Retry is available.';
    return;
  }
  ...
  if (kind === 'success') {
    ...
    ui.weatherSuccess.hidden = false;
    renderReadableWeather(mappedSignal, 'live');
...
function renderSearchState() {
  ...
  if (state.selectedLocation) {
    ...
    ui.starterStatus.textContent = state.selectedScenario
      ? 'Location and scenario selected. You can now fetch weather evidence.'
      : 'Location selected. Choose a fictional scenario to enable weather fetch.';
    return;
  }
...
function renderSelectedLocation() {
  ...
  ui.fetchWeatherButton.disabled = state.weather.kind === 'loading' || state.selectedScenario === '';
}
...
function onUnitSystemChange(event) {
  state.selectedUnitSystem = event.target.value;
}
...
function bindEvents() {
  ...
  ui.unitSystemSelect.addEventListener('change', onUnitSystemChange);
  ui.fetchWeatherButton.addEventListener('click', fetchWeatherForSelectedLocation);
  ui.weatherRetry.addEventListener('click', fetchWeatherForSelectedLocation);
}

function init() {
  state.selectedScenario = ui.scenarioSelect.value;
  state.selectedUnitSystem = ui.unitSystemSelect.value;
  bindEvents();
  renderSearchState();
  renderSelectedLocation();
  renderWeatherState();
