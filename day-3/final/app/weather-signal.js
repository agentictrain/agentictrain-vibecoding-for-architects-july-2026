(function weatherSignalModule(root) {
  'use strict';
  ...
  const ALLOWED_UNITS = {
    current: {
      temperature: ['degC', 'degF', '°C', '°F'],
      apparentTemperature: ['degC', 'degF', '°C', '°F'],
      humidity: ['%'],
      precipitation: ['mm', 'in'],
    ...
    daily: {
      temperatureMax: ['degC', 'degF', '°C', '°F'],
      temperatureMin: ['degC', 'degF', '°C', '°F'],
      precipitationSum: ['mm', 'in'],
      precipitationProbabilityMax: ['%'],
      weatherCode: ['wmo code'],
      windSpeedMax: ['km/h', 'mph'],
      sunrise: ['iso8601'],
      sunset: ['iso8601']
    }
  };
  ...
  function extractUnits(unitsBlock, fields, sectionName) {
    ...
    fields.forEach(([sourceKey, targetKey]) => {
      ...
      ensureString(unitValue, `${sectionName} missing unit for ${sourceKey}`);
        ...
        Array.isArray(ALLOWED_UNITS[sectionName][targetKey])
          && ALLOWED_UNITS[sectionName][targetKey].includes(unitValue),
        `${sectionName} has unknown unit for ${sourceKey}`
      );
