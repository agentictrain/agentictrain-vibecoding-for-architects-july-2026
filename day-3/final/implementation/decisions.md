...
## 2026-07-22 discrepancy remediation pass

- decision: Prioritize spec and ADR wording when plan text conflicts on fallback trigger behavior.
- reason: Spec and ADR define expected product behavior and terminology boundary for acceptance.
- alternatives considered: Rewrite behavior to plan-only explicit fallback loading flow.
- expected impact: Keeps implementation aligned with acceptance criteria while preserving explicit retry handling.
- whether human approval is still needed: Yes, confirm this precedence rule for future edits.

- decision: Add strict known-unit validation in mapper plus Node tests before further feature expansion.
- reason: Unknown unit rejection was a blocking contract gap and tests were missing from the implementation plan.
- alternatives considered: Keep permissive non-empty unit checks until later.
- expected impact: Improves contract safety and reduces regression risk for weather mapping.
- whether human approval is still needed: No, this directly closes identified blocking findings.
