(function weatherSignalModule(global) {
  ...
  const ALLOWED_UNITS = {
    ...
    wind_speed_10m: ['km/h', 'mph', 'm/s', 'kn'],
    wind_direction_10m: ['°'],
    wind_gusts_10m: ['km/h', 'mph', 'm/s', 'kn'],
    cloud_cover: ['%'],
    ...
    wind_speed_10m_max: ['km/h', 'mph', 'm/s', 'kn'],
    sunrise: ['iso8601'],
    sunset: ['iso8601']
  };
  ...
  function extractUnits(unitsBlock, fields, sectionName) {
    ...
    fields.forEach(([sourceKey, targetKey]) => {
      ...
      const allowedUnits = ALLOWED_UNITS[sourceKey] || [];
      ensure(allowedUnits.includes(unitValue), `${sectionName} unknown unit for ${sourceKey}: ${unitValue}`);
